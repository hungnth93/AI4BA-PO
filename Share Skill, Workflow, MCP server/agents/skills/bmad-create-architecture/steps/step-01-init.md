...
- **Security & compliance (khuyến nghị dự án production):** Mặc định **gom trong** `{planning_artifacts}/architecture.md` — section kiểu `## Security & Compliance Profile` (nội dung lấy từ `./data/architecture-security-profile.template.md` của skill, dán vào khi khởi tạo hoặc ở bước 4). **Không** bắt buộc file `architecture-security-profile.md` riêng. Chỉ discover thêm nếu tồn tại: `architecture-security-profile.md`, `*security-profile*.md`, hoặc shard `architecture/…` có index.
