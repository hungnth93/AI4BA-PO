...
- Mặc định ghi baseline bảo mật **trong** `{planning_artifacts}/architecture.md` (một section, ví dụ `## Security & Compliance Profile`). Có thể lấy khung nội dung từ `./data/architecture-security-profile.template.md` của skill (dán và điền). Shard hoặc file `architecture-security-profile.md` riêng chỉ khi team chủ động tách sau.
- Khi thảo luận category này, output ghi vào document phải bao phủ tối thiểu: **mục tiêu ASVS / level**, **attack surface**, **authN/authZ**, **bảo vệ API** (TLS, rate limit, headers nếu có), **secrets/config**, **mã hóa dữ liệu nhạy cảm**, **logging/audit** (không lộ PII), **mối liên hệ với NFR/PRD** (PII, thanh toán, API public…).

**Luồng thực hiện:**

- **Đã có section security trong `architecture.md` (hoặc đã load profile/shard cũ):** mở đầu category bằng tóm tắt **ASVS / data classes / attack surface** từ đó; mọi gợi ý phải **bám baseline** và ghi rõ mục ASVS hoặc NFR nào được đáp ứng. Control mà starter chưa đáp ứng → quyết định hoặc **TODO kiến trúc** có lý do.
- **Chưa có section security:** suy luận từ PRD; **nhắc user** thêm section vào `architecture.md` bằng template trên (một file); không bắt buộc tách file riêng.

**Chủ đề cần quyết định trong category:**
