...
**HTTP / typed API backends:** If the stack exposes **HTTP APIs** (REST, GraphQL, etc.) behind a **typed boundary** (NestJS DTOs, Zod schemas, tRPC procedures, ASP.NET records, …), the patterns you save MUST include explicit **Request** and **Response** contract rules so agents do not implement “validate input only” and return ad-hoc shapes. If the backend has **no public HTTP API**, still add a short line under the DTO pattern section: **N/A — no HTTP API boundary**.
...
- Request vs Response contract at the HTTP boundary (DTO/schema classes vs internal `type` only)
...
**API boundary contracts (DTO / schema — multi-agent consistency):**

- **When:** Any service that accepts HTTP requests and returns structured payloads to clients. Omit detail only if architecture truly has no HTTP API (then mark N/A in the written patterns).
- **NestJS (default framing):** Request: class DTOs under `src/modules/<domain>/dto/` with global validation for query/body/params. Response: class DTOs (or OpenAPI-documented equivalents) for the object inside the success `data` envelope — list endpoints define **item DTO + wrapper** (e.g. `items`, `total`). Map ORM/entity → Response DTO in the service layer; controllers do not expose raw persistence models as the public contract.
- **Other stacks:** Name the equivalent (Zod input/output, tRPC router IO, Kotlin data classes, etc.) using the same rule: **named input contract + named output contract**, colocated per domain module.
- **Rollout:** New endpoints require both contracts; legacy endpoints gain Response contracts when touched.
- **Anti-pattern:** Public response shape exists only as an internal `type` in a service file with no shared boundary artifact — breaks FE alignment and OpenAPI.

**Facilitator obligation:** For NestJS (or similar), the markdown appended to the architecture document in this step **must** include a `### Pattern DTO — Request & Response` section (or explicit N/A). Do not skip silently.

**Facilitator obligation (layering):** For backend module architectures (NestJS and similar), the markdown in this step **must** include explicit layering rules stating:
- route definitions are in controllers (decorators), not in module files;
- service owns business logic;
- repository owns CRUD/query with ORM;
- controllers do not access ORM directly;
- if repository is not yet separated, mark as legacy with migration plan.

**Project-context alignment note (mandatory handoff):**
- If architecture defines HTTP boundary contracts (Request/Response DTOs or stack-equivalent), record a short sync instruction in this step output so whoever refreshes `project-context.md` copies the same boundary rules.
- If no HTTP API boundary exists, record explicit N/A to avoid silent ambiguity later.
...
### Pattern DTO — Request & Response (API boundary)

{{request_and_response_dto_rules_or_explicit_n_a_for_non_http}}
...
- {{mandatory_pattern_2}}
- {{mandatory_pattern_3}}
- For HTTP APIs: use paired **Request + Response** boundary contracts per endpoint or resource group; map persistence → Response contract before returning `data` (unless architecture marks this slice N/A).
...
✅ If an HTTP API is in scope, patterns include explicit Request/Response boundary rules (or documented N/A)
...
❌ **CRITICAL**: Omitting Request/Response boundary rules when the stack includes a typed HTTP backend (e.g. NestJS) — causes inconsistent public contracts
