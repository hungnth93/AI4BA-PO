...
- For HTTP API backends: do patterns define **both** Request and Response boundary contracts (or explicit N/A)?
...
- **If security baseline exists** (section trong `architecture.md`, hoặc file profile/shard đã load): Does the architecture explicitly address the stated **ASVS target level** (map major areas: auth/session, access control, validation, crypto, logging, configuration, API security)? List **gaps** or **deferred controls** with rationale; do not claim full ASVS compliance unless decisions + patterns substantiate it.
- **If no security section/profile content was loaded:** recommend appending a **Security & Compliance** section to `{planning_artifacts}/architecture.md` using `./data/architecture-security-profile.template.md` (single document default; optional separate file or shard only if team wants).
...
- For typed HTTP stacks: is the **DTO / API boundary** section present and actionable (not missing Response-side rules)?
- For backend layering outputs: are route/controller/service/repository/database responsibilities explicitly separated and non-contradictory?
- If `project-context.md` is used/regenerated in this project, is there an explicit sync note to carry over Request/Response boundary rules from architecture?
...
- [x] API boundary: Request + Response contracts documented (or N/A if no HTTP API)
...
- [x] Backend layered mapping documented (Route in controller decorators, Module as wiring, Service business logic, Repository data access, Database layer explicit) or N/A
...
- For HTTP APIs: implement **paired Request + Response** boundary types as documented; keep `project-context.md` in sync with those rules when it is regenerated
