...
Đảm bảo file tuân thủ khung **§1–§10** từ `project-context-template.md`:

- §1 Tech Stack đã điền cụ thể (không còn placeholder italic).
- §2–§4, §6–§9: subsection merge từ Step 2 không còn `_Document sau bước generate._` trừ khi chủ động để trống.
- §10 How To Use đã có sẵn trong template — chỉnh wording nếu cần, không duplicate mục Usage thứ hai.
- Loại bỏ trùng lặp, gộp bullet dài dòng; giữ rule có giá trị riêng cho LLM.
...
(Facilitator gán `rule_count` = số quy tắc actionable ước lượng, không để 0 nếu đã đếm được.)

**Usage (§10):**
Template đã có `## 10) How To Use`. Chỉ bổ sung/sửa nếu thiếu; không append thêm một khối Usage trùng ý.
