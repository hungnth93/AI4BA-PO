...
**Backend HTTP API (NestJS, Fastify, tRPC, ASP.NET, … — if applicable):**
- When **`bmad-create-architecture`** produced planning artifacts, read the **Pattern DTO — Request & Response** (or equivalent) section and **copy those boundary rules** into the generated `project-context.md` (Language + Framework sections) so coding agents match architecture.
- If no architecture shard exists, infer from the repo: public HTTP endpoints should have **paired input + output contracts** (DTO/schema), not only validated request while response shape lives as an internal `type` in a service.
...
File `project-context.md` đã có khung **§1–§10** từ `project-context-template.md`. Với **mỗi category** dưới đây, chuẩn bị nội dung **gọn** rồi **merge** vào đúng chỗ — thay dòng `_Document sau bước generate._` / placeholder italic, hoặc bổ sung bullet dưới subsection — **không** chèn thêm `## Technology Stack & Versions` hay lặp heading đã có.

#### Map category → section

| Category trong workflow | Merge vào |
|-------------------------|-----------|
| 1. Technology Stack & Versions | `## 1) Tech Stack Snapshot` — thay các bullet italic bằng stack thật + version |
| 2. Language-Specific Rules | `## 2) Language & Contract Rules` → `### Language-specific` |
| 3. Framework-Specific Rules | `### Framework-specific — React/FE` (§3) và/hoặc `### Framework-specific — NestJS/BE` (§4) tùy stack |
| 4. Testing Rules | `## 7) Testing Rules` → `### Testing` |
| 5. Code Quality & Style | `## 6) Code Size Guardrails` → `### Code Quality & Style` |
| 6. Development Workflow | `## 8) Delivery Quality Gates` → `### Development Workflow` |
| 7. Gotchas / Anti-patterns | `## 9) Anti-Patterns` → `### Critical Don't-Miss` |

Khi trình bày draft cho user, có thể hiển thị **chỉ** block markdown sẽ được merge (ví dụ chỉ các bullet §1), không cần lặp toàn bộ file.

#### Content shape (ví dụ từng nhóm)

```markdown
# §1 — chỉ liệt kê bullet stack, không thêm heading cấp 2 mới
- Backend: Node 22, NestJS 11, Prisma 6, …
```
...
## UPDATE PROJECT CONTEXT (merge, không append heading trùng):

Khi user chọn **C** cho một category: đọc `{output_folder}/project-context.md`, **sửa đúng section/subsection** theo bảng map ở bước 8, giữ nguyên các § baseline (DTO, envelope, anti-pattern cố định) trừ khi user yêu cầu đổi. Không append khối `## Technology Stack & Versions` / `## Critical Implementation Rules` kiểu cũ.
